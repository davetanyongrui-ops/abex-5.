                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12 max-w-6xl mx-auto">
                    {certifications.map((cert) => (
                        <div key={cert.name} className="flex flex-col p-10 rounded-2xl bg-white border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1 group items-center text-center">
                            <div className="h-36 w-full relative mb-10 flex items-center justify-center transition-all duration-500">
                                <Image
                                    src={cert.logo}
                                    alt={cert.name}
                                    width={200}
                                    height={200}
                                    className="object-contain h-28 w-auto mix-blend-multiply transition-transform duration-500 group-hover:scale-110"
                                />
                            </div>
